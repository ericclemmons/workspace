...
- **Agent edit guard:** blocks edits outside `worktrees/*`, blocks meta-infrastructure (`.githooks/`, `AGENTS.md`, `.claude/`, `opencode.json`, `mise.toml`, `mise-tasks/*`), allows valid paths.
...
- **End-to-end:** real `add → branch → edit → push --no-pr → pull` against a local fake upstream.
